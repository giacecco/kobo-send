...
- `automator/*.workflow` — Finder Quick Action ("Send to Kobo", files) on the Mac. A thin `Run Shell Script` wrapper calling `~/.bin/kobo-send-client.sh`. Installed by copying into `~/Library/Services/`.
- No repo folder for the iOS Shortcuts, or for the macOS "Send Page to Kobo" Shortcut — they're built directly in the Shortcuts app (see README's Setup section) since Shortcuts has no plain-text export format worth tracking.
...
- **Safari stopped surfacing legacy Automator `NSServices` for whole-page actions** on modern macOS (confirmed broken on 26.6.2; worked as recently as ~a month prior with no deprecation notice). The old Safari "Send Page to Kobo" Service is retired — don't recreate it as an Automator Service. Its replacement is a macOS Shortcut (Share Sheet, "Run Shell Script" calling `kobo-send-client.sh`), built directly in the Shortcuts app per the README, not tracked in this repo.
