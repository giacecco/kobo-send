...
- Title and author are determined by a small, dedicated `claude` call (always on Sonnet) given just the first page's text (extracted locally with `pdftotext`, no AI needed for that), the PDF's embedded metadata (via `pdfinfo`), and the filename — in that priority order, never guessing beyond what's actually supported, and never accepting a bare first name as an author. The much larger job of converting the rest of the document is a separate call that doesn't touch title/author at all, tried on Haiku first and escalating to Sonnet then Opus only if that fails — keeping the accuracy-sensitive part cheap-but-reliable and the bulk conversion cheap-by-default.
